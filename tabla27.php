...
$statement = $conexion->prepare("SELECT regisalones.id, regisalones.estado, regisalones.fecha, regisalones.mac, salones.nombre_salones AS salones FROM regisalones INNER JOIN salones ON regisalones.salones_id = salones.id WHERE salones.id = 27 ORDER BY id DESC LIMIT $inicio, $fin");
$statement->execute();
$resultados = $statement->fetchAll();

//Saber si el movimiento es 0 o 1 para asignar color
$consultarMovimiento = $conexion->prepare("SELECT * FROM regisalones WHERE salones_id = 27 ORDER BY id DESC LIMIT 1");
